...
class HDKitCalculator {
  constructor() {
    ...
    try {
      ...
      console.warn('⚠️ Ephemeris path not found, using built-in data');
    }
    
    this.flag = swisseph.SEFLG_SPEED;
  ...
  async getPlanetPosition(julianDay, planetId) {
    return new Promise((resolve, reject) => {
      swisseph.swe_calc_ut(julianDay, planetId, this.flag, (result) => {
        if (result.error) {
  ...
  async findDesignDate(personalityJulianDay, sunLongitude) {
    ...
    // Start search approximately 88-92 days before birth
    let searchJulianDay = personalityJulianDay - 89;
    let iterations = 0;
    const maxIterations = 1000;
    
    while (iterations < maxIterations) {
      try {
        const sunPosition = await this.getPlanetPosition(searchJulianDay, swisseph.SE_SUN);
        const offset = Math.abs(sunLongitude - sunPosition.longitude);
        
        // Handle crossing 0° boundary
        const adjustedOffset = offset > 180 ? 360 - offset : offset;
        
        // Check if we're close to 88° (within 0.1°)
        if (Math.abs(adjustedOffset - 88) < 0.1) {
          console.log(`✅ Design date found at Julian Day ${searchJulianDay} (offset: ${adjustedOffset}°)`);
          return searchJulianDay;
        }
        
        // Move forward in smaller increments as we get closer
        const increment = adjustedOffset > 90 ? 0.1 : 0.01;
        searchJulianDay += increment;
        
      } catch (error) {
        console.error('❌ Error in Design date search:', error);
        break;
      }
    ...
    // Fallback: return approximately 88.5 days before
  ...
  eclipticToGate(eclipticLongitude) {
    ...
    // Calculate position within the 360° wheel
    const percentageThrough = adjustedLongitude / 360;
    
    // Get gate from the specific HD order
